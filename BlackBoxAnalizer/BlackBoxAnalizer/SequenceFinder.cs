...
namespace BlackBoxAnalizer
{
    class SequenceFinder
    {
        ...
        public SequenceFinder(Pin[] pins)
        {
            ...
            lines = pins[0].TimingLines;
            boolStreams = new BoolStream[inPinCount][];
            counterPin = new bool[inPinCount];
            for (int i = 0; i < inPinCount; i++)
                if (counterPin[i] = pins[i].TimingEnabled) boolStreams[i] = pins[i].BoolStream;
        ...
        int[] bubleSegmentSort(int l)
        {
            ...
            for (int i = cp - 1; i >= 0; i--)
            {
                ...
                if (width[sort[i]] == 4) break;
                if ((edges[sort[i]] == 1) && (dutys[sort[i]] == 2))
                {
